...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    ...
    vAlign="top">
     <form action="${ctx }/manage/role-manage.action" method="post">
<table>
	<tr>
		<td height="23">角色名</td>
		<td><input type="text" name="roleName" id="roleName" value="${roleName }"/></td>
		<td>开始时间</td>
	...
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/role-manage.action${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
                <TR >                
                  ...
                  <a href="${ctx }/manage/role-manage.action${order }id:asc">升</a>
                  <a href="${ctx }/manage/role-manage.action${order }id:desc">降</a></TH>
                  ...
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/role-manage.action${order }createTime:asc">升</a>
                  <a href="${ctx }/manage/role-manage.action${order }createTime:desc">降</a>
				 </TH>
				  <TH class=gridviewHeader scope=col>更新缓存（需重新登录）</TH>
            ...
            href="${ctx }/manage/role-manage!toSetActionByRole.action?id=${role.id}">权限</A>
            -<A class=cmdField 
            href="${ctx }/manage/role-manage!toSetMenuByRole.action?id=${role.id}">菜单</A>
            </TD>
                  <TD class=gridViewItem>${role.createTime}</A></TD>
                   <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/role-manage!updateCacheByRole.action?id=${role.id}" target="_parent">更新角色</A>
          
            </TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/role-manage!toAlter.action?id=${role.id}">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/role-manage!delete.action?id=${role.id}';">删除</A> </TD>
                </TR>
             </c:forEach>
            </TABLE>
            ${tag }
