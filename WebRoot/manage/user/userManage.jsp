...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    ...
    vAlign="top">
     <form action="${ctx }/manage/user-manage.action" method="post">
<table>
	<tr>
		<td height="23">用户名</td>
		<td><input type="text" name="userName" id="roleName" value="${userName }"/></td>
		<td>开始时间</td>
	...
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/user-manage.action${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
               <TR >                
                  ...
                  <a href="${ctx }/manage/user-manage.action${order }id:asc">升序</a>
                  <a href="${ctx }/manage/user-manage.action${order }id:desc">降序</a></TH>
            ...
            href="${ctx }/manage/role-manage.action?id=${user.id}">所有角色</A>                  </TH>
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/user-manage.action${order }createTime:asc">升序</a>
                  <a href="${ctx }/manage/user-manage.action${order }createTime:desc">降序</a>				 </TH>
            ...
            href="${ctx }/manage/user-manage!toSetRoleByUser.action?id=${user.id}">分配角色</A></TD>
                  <TD class=gridViewItem>${user.createTime}</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/user-manage!toAlter.action?id=${user.id}">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/user-manage!delete.action?id=${user.id}';">删除</A> </TD>
                </TR>
             </c:forEach>
            </TABLE>
            ${tag }
