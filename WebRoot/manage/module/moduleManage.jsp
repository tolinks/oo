...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    ...
    vAlign="top">
     <form action="${ctx }/manage/module-manage.action" method="post">
<table>
	<tr>
		<td height="23">模块名</td>
		<td><input type="text" name="moduleName" id="moduleName" value="${moduleName }"/></td>
		<td>开始时间</td>
	...
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/module-manage.action${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
                <TR >                
                  ...
                  <a href="${ctx }/manage/module-manage.action${order }id:asc">升序</a>
                  <a href="${ctx }/manage/module-manage.action${order }id:desc">降序</a></TH>
                  <TH class=gridViewHeader scope=col>角色名</TH>
                  <TH class=gridViewHeader scope=col>角色详情</TH>
            
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/module-manage.action${order }createTime:asc">升序</a>
                  <a href="${ctx }/manage/module-manage.action${order }createTime:desc">降序</a>
				 </TH>
                ...
                <TR>
                 ...
                 <a href="${ctx }/manage/action-manage.action?moduleId=${module.id}"> ${module.moduleName}</a>
                  </TD>
            ...
            href="${ctx }/manage/module-manage!toAlter.action?id=${module.id}">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?')){
            			if(${fn:length(module.actions)>0}){
            				alert('请先移除相关的权限再删除');
            			}else{
            				location.href='${ctx }/manage/module-manage!delete.action?id=${module.id}';
            			}
            		}
            		">删除</A> </TD>
                </TR>
