...
$("#imageUrl").formValidator({
	...
	<BODY
								...
								<tr>
									...
									<td width="190">
										<input name="menuName" type="text" id="menuName"
											style="width: 120px" value="" size="50" />
									</td>
									<td width="284">
		...
		<c:forEach items="${menuList}" var="menu" varStatus="s">
		   ...
		   d.add(${menu.id}, ${menu.parentId}, "<input type='radio' name='parentId' value='${menu.id}'<c:if test="${menu.id==checkMenuId}">checked</c:if>>${menu.menuName }");       	
		</c:forEach>
