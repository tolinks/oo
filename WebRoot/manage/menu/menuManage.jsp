...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    ...
    vAlign="top">
     <form action="${ctx }/manage/menu-manage.action" method="post">
<table>
	<tr>
		<td height="23">菜单名</td>
		<td><input type="text" name="menuName" id="menuName" value="${menuName }"/></td>
		<td>开始时间</td>
		...
		<a href="${ctx }/manage/menu-manage!toSave.action">增加</a></td>
	</tr>
	<tr>
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/menu-manage.action${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
                <TR >                
                  ...
                  <a href="${ctx }/manage/menu-manage.action${order }id:asc">升序</a>
                  <a href="${ctx }/manage/menu-manage.action${order }id:desc">降序</a></TH>
                  <TH class=gridViewHeader scope=col>菜单名</TH>
                  <TH class=gridViewHeader scope=col>菜单排序
                  <a href="${ctx }/manage/menu-manage.action${order }orders:asc">升</a>
                  <a href="${ctx }/manage/menu-manage.action${order }orders:desc">降</a>
                  </TH>
                  <TH class=gridViewHeader scope=col>权限</TH>
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/menu-manage.action${order }createTime:asc">升</a>
                  <a href="${ctx }/manage/menu-manage.action${order }createTime:desc">降</a>
				 </TH>
            ...
            href="${ctx }/manage/menu-manage!toSave.action?id=${menu.id}">添加子菜单</A></TD>
                  <TD class=gridViewItem>${menu.createTime}</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/menu-manage!toAlter.action?id=${menu.id}">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/menu-manage!delete.action?id=${menu.id}';">删除</A> </TD>
                </TR>
             </c:forEach>
            </TABLE>
            ${tag }
