...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    ...
    vAlign="top">
     <form action="${ctx }/manage/action-manage.action" method="post">
<table>
	<tr>
		<td height="23">
		...
		<td>权限名<input type="text" name="actionName" id="actionName" value="${actionName }"/></td>
		<td>开始时间</td>
		...
		<a href="${ctx }/manage/action-manage!toSave.action">增加</a></td>
	</tr>
	<tr>
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/action-manage.action${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
                <TR >                
                  ...
                  <a href="${ctx }/manage/action-manage.action${order }id:asc">升</a>
                  <a href="${ctx }/manage/action-manage.action${order }id:desc">降</a></TH>
                  <TH class=gridViewHeader scope=col>权限名</TH>
                  <TH class=gridViewHeader scope=col>权限详情</TH>
                  <TH class=gridViewHeader scope=col>排序
                  <a href="${ctx }/manage/action-manage.action${order }orders:asc">升</a>
                  <a href="${ctx }/manage/action-manage.action${order }orders:desc">降</a>
                  </TH>
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/action-manage.action${order }createTime:asc">升</a>
                  <a href="${ctx }/manage/action-manage.action${order }createTime:desc">降</a>
				 </TH>
            ...
            href="${ctx }/manage/action-manage.action?moduleId=${action.module.id}"> ${action.module.moduleName}</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/action-manage!toAlter.action?id=${action.id}">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/action-manage!delete.action?id=${action.id}';">删除</A> </TD>
                </TR>
             </c:forEach>
            </TABLE>
            ${tag }
